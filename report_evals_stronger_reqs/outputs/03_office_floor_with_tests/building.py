...
def build():
    ...
    #
    # IMPORTANT: Interior walls connect to exterior walls at these Y positions:
    # - Corridor South at Y=4000 (connects to east wall)
    # - Corridor North at Y=5500 (connects to east and west walls)
    # - Office Corridor at Y=9000 (connects to west wall)
    #
    # Windows must avoid these connection points with 300mm clearance.
    # Window width is 1800mm, so window spans offset to offset+1800.

    # East wall: connections at Y=4000 and Y=5500
    # Safe zones: Y=300-3700 (before 4000-300) and Y=6100-11700 (after 5500+300+300)
    east_win_1 = Window(
        types["large_window"],
        wall_east,
        offset=500,  # Y=500-2300, safe (before Y=4000)
        name="Win_E1",
        mark="W-05",
    )
    ...
    east_win_2 = Window(
        types["large_window"],
        wall_east,
        offset=6500,  # Y=6500-8300, safe (after Y=5500+300)
        name="Win_E2",
        mark="W-06",
    )
    ...
    east_win_3 = Window(
        types["large_window"],
        wall_east,
        offset=9500,  # Y=9500-11300, safe (before Y=12000-300)
        name="Win_E3",
        mark="W-07",
    )
    ...
    # Connections at Y=5500 (offset=6500) and Y=9000 (offset=3000)
    # Safe zones in offset terms:
    # - offset 300-2700 (Y=11700-9300, before Y=9000)
    # - offset 3600-5900 (Y=8400-6100, between connections)
    # - offset 7100-11700 (Y=4900-300, after Y=5500)
    west_win_1 = Window(
        types["large_window"],
        wall_west,
        offset=500,  # offset 500-2300, Y=11500-9700, safe (before Y=9000)
        name="Win_W1",
        mark="W-08",
    )
    ...
    west_win_2 = Window(
        types["large_window"],
        wall_west,
        offset=4000,  # offset 4000-5800, Y=8000-6200, safe (between connections)
        name="Win_W2",
        mark="W-09",
    )
    ...
    west_win_3 = Window(
        types["large_window"],
        wall_west,
        offset=7500,  # offset 7500-9300, Y=4500-2700, safe (after Y=5500)
        name="Win_W3",
        mark="W-10",
    )
...
def main():
    ...
    # Create all sheets
    sheets = [
        create_drawing_sheet("Ground Floor Plan", "A-101", floor_plan_result),
        create_drawing_sheet("North Elevation", "A-201", north_elev_result),
        create_drawing_sheet("South Elevation", "A-202", south_elev_result),
        create_drawing_sheet("East Elevation", "A-203", east_elev_result),
        create_drawing_sheet("West Elevation", "A-204", west_elev_result),
        create_drawing_sheet("Section A-A", "A-301", section_result),
    ]

    # Export all sheets to a single PDF
    pdf_path = out_dir / "03_office_floor_drawing_set.pdf"
    pdf_exporter.export_sheets(sheets, str(pdf_path))

    print(f"  Saved: {pdf_path.name} ({len(sheets)} pages)")
