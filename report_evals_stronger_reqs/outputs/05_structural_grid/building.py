...
def create_building_envelope(level, types):
    ...
    # Exterior walls
    wall_south = Wall(ext_wall, (0, 0), (BUILDING_LENGTH, 0), level, name="Ext_South")
    wall_east = Wall(
        ext_wall, (BUILDING_LENGTH, 0), (BUILDING_LENGTH, BUILDING_WIDTH), level, name="Ext_East"
    )
    wall_north = Wall(
        ext_wall, (BUILDING_LENGTH, BUILDING_WIDTH), (0, BUILDING_WIDTH), level, name="Ext_North"
    )
    wall_west = Wall(ext_wall, (0, BUILDING_WIDTH), (0, 0), level, name="Ext_West")
    walls.extend([wall_south, wall_east, wall_north, wall_west])

    # Loading door on south wall (4m x 4m, centered)
    # Wall length is 24m, door is 4m, so offset to center is (24000 - 4000) / 2 = 10000
    loading_door = Door(
        types["loading_door"],
        wall_south,
        offset=BUILDING_LENGTH / 2 - types["loading_door"].width / 2,
        name="Loading_Door",
        mark="D-01",
    )
