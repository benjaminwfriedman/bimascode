...
def build():
    ...
    # Wall starts at X=0, window at offset 300mm
...
def main():
    ...
    # === Generate PDF drawing set ===
    print("\nGenerating PDF drawing set...")

    # Create sheet with floor plan
    sheet = Sheet(
        size=SheetSize.ARCH_D,  # 24" x 36"
        number="A-101",
        name="Floor Plan",
    ...
    # Add floor plan viewport
    sheet.add_viewport(
        plan_result,
        position=(304.8, 500),  # Center of sheet
        scale=ViewScale.SCALE_1_100,
        name="Ground Floor Plan",
    )
